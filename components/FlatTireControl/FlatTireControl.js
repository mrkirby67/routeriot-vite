...
let flatTireToggleCleanup = null;

function setupFlatTireCollapsible() {
  const toggleBtn = document.getElementById('toggle-flat-tire-btn');
  const panel = document.getElementById('flat-tire-panel');
  ...
  // Start collapsed for a tidier control panel.
  applyState(false);
  ...
  return `
    <div class="${styles.controlSection}">
      <div class="${styles.headerRow}">
        ...
        <button type="button" id="toggle-flat-tire-btn" class="${styles.secondaryBtn}">Expand ▼</button>
      </div>

      <div id="flat-tire-panel" style="display:none;">
...
export async function initializeFlatTireControl() {
  flatTireToggleCleanup?.();
  flatTireToggleCleanup = setupFlatTireCollapsible();
...
export function teardownFlatTireControl(reason = 'manual') {
  flatTireToggleCleanup?.(reason);
  flatTireToggleCleanup = null;
