...
// ---------------------------------------------------------------------------
// 🪰 Load Settings (starting strikes, cooldowns, rewards, etc.)
// ---------------------------------------------------------------------------
export async function getBugStrikeSettings() {
  try {
    ...
    console.error("❌ Error fetching BugStrike settings:", err);
    return {};
  }
}

// ---------------------------------------------------------------------------
// 🧠 Initialize Strikes for All Teams
...
export async function initializeBugStrikesForTeams(teamNames = []) {
  try {
    ...
    console.log(`✅ Initialized Bug Strikes for ${teamNames.length} teams.`);
  } catch (err) {
    console.error("❌ Error initializing Bug Strikes:", err);
  }
}

// ---------------------------------------------------------------------------
// 🎁 Grant Strike to a Team (e.g. from a reward trigger)
// ---------------------------------------------------------------------------
export async function grantBugStrike(teamName, source = 'manual') {
  try {
    ...
    console.log(`🎁 Bug Strike granted to ${teamName} (${source}).`);
  } catch (err) {
    console.error(`❌ Failed to grant Bug Strike to ${teamName}:`, err);
  }
}
...
export async function useBugStrike(fromTeam, targetTeam) {
  try {
    ...
    const snap = await getDoc(teamRef);
    if (!snap.exists()) {
      console.warn(`⚠️ Team ${fromTeam} has no strike data.`);
    ...
    console.error(`❌ Error using Bug Strike from ${fromTeam}:`, err);
    return { success: false, reason: 'error' };
  }
}

// ---------------------------------------------------------------------------
// 🧹 Reset All Bug Strikes (e.g., when game is reset)
// ---------------------------------------------------------------------------
export async function resetBugStrikes() {
  try {
    ...
    console.log('🧹 All Bug Strikes reset.');
  } catch (err) {
    console.error('❌ Failed to reset Bug Strikes:', err);
  }
}

// ---------------------------------------------------------------------------
// 📊 Utility: Get Summary of All Teams (for Control display)
// ---------------------------------------------------------------------------
