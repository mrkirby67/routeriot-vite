...
export async function runFlatTireZoneDiagnostics() {
  console.log('🚦 [Diagnostics] Checking zones for Flat Tire compatibility...');

  let snapshot;
  try {
    snapshot = await getDocs(collection(db, 'zones'));
  } catch (err) {
    console.error('🔥 Diagnostics failed to load zones:', err);
    return;
  }

  if (snapshot.empty) {
    console.warn('⚠️ No zones found in Firestore. Flat Tire dropdown will be empty.');
    return;
  }
  ...
  snapshot.forEach((docSnap) => {
    ...
    if (hasCoords && isDepot) {
      ...
      invalid += 1;
      console.warn(
        `❌ Invalid zone → ${name} — type="${data.type ?? 'missing'}", latitude=${lat}, longitude=${lng}`
      );
    }
  });

  console.log(`📊 Scan complete → ${valid} depot(s) valid, ${invalid} zone(s) missing requirements.`);
  console.log('🧩 Reminder: Flat Tire dropdown only lists zones with { type: "depot", latitude, longitude }.');
}
