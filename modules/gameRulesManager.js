...
export async function startGame(gameDuration = 120) {
  ...
  const stateSnap = await getDoc(doc(db, 'game', 'gameState'));
  const currentStatus = stateSnap.exists() ? String(stateSnap.data().status || '').toLowerCase() : 'waiting';
  if (currentStatus === 'active' || currentStatus === 'paused') {
    alert('⚠️ Game already running — use Pause/Resume or End Game instead of Start.');
    return;
  }
...
export async function clearAllCollectionsAndReset() {
  ...
  showFlashMessage("🧹 Clearing Firestore collections...");
  ...
  try {
    for (const colName of targets) {
      ...
      console.log(`✅ Cleared ${colName} (${deletions.length} docs)`);
    }

    await resetGameState();
    showFlashMessage("✅ All collections cleared and game reset.");
  } catch (err) {
    console.error("❌ clearAllCollectionsAndReset failed:", err);
    showFlashMessage("⚠️ Reset failed – see console for details.");
  }
}
