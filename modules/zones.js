...
import { broadcastChallenge, updateTeamLocation } from './zonesFirestore.js';
...
export async function initializeZones(teamName) {
  ...
  registerZoneListener('gameState', onSnapshot(doc(db, 'game', 'gameState'), (gameSnap) => {
    ...
    if (['finished', 'ended', 'over'].includes(data.status)) {
      showFinalStandings();
    }
        ...
        navigator.geolocation.getCurrentPosition(
          async (pos) => {
            ...
            if (dist <= targetRadiusKm + pos.coords.accuracy / 1000) {
              ...
              await updateTeamLocation(currentTeamName, zoneData.name);
